...
def send_welcome(message):
    ...
    welcome_text = (
        "🎓 ———————————————————— 🎓\n"
        "**КУРСОВАЯ РАБОТА**\n"
        "Тема: «Разработка мультиагентной системы на базе LLM»\n"
        "——————————————————————\n\n"
        "🤖 **Разработчик:** студент группы №32\n"
        ...
        for action in lines:
            if action.startswith("CALC:"):
                ...
                if "Ошибка" not in search_data and "не дал результатов" not in search_data:
                    ...
                    summary_prompt = [
                        SystemMessage(content="Ты — полезный ассистент. Оформи факты в красивый, структурированный текстовый блок на русском языке. Пиши только суть, без лишней воды."),
