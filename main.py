...
def handle_agent_message(message):
    try:
        ...
        system_instruction = (
            "Ты — минималистичный ИИ-агент, управляющий инструментами. Отвечай кратко, строго по делу, без воды.\n"
            "Если пользователю нужно что-то посчитать, узнать погоду, найти информацию или курс валют — "
            "вызывай соответствующий инструмент из списка. Не пиши формулы текстом, используй инструменты."
        )
        ...
        bot.reply_to(message, "Произошла ошибка при обработке запроса.", reply_markup=get_main_keyboard())
